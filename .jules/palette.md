...
## 2024-05-18 - Improve link-based overlay buttons
**Learning:** Custom link-based overlay buttons (like the floating 'Run' button on code blocks) do not naturally inherit standard button focus outlines and lack descriptive context when opening new tabs.
**Action:** When implementing custom link buttons that float on elements, ensure you add explicit `:focus-visible` styles matching their custom border radius and add an `aria-label` describing the action (e.g. including "opens in a new tab") to supplement brief visual text.
