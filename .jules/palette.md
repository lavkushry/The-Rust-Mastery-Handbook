...
## 2024-06-10 - Screen Reader Redundancy on CSS Pseudo-elements
**Learning:** Avoid using CSS pseudo-elements (like `::after` or `::before`) to inject textual hints or keyboard shortcuts into interactive UI components, as they cannot be individually hidden from screen readers using `aria-hidden` and cause redundant announcements.
**Action:** Inject actual DOM elements with `aria-hidden="true"` instead for visual hints that should be ignored by screen readers.
