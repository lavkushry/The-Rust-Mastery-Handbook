...
## 2024-07-04 - Floating Action Button Semantic Accessibility
**Learning:** Custom link-based overlay buttons (like 'Run' buttons floating over code blocks) that open new tabs often have brief visible text (e.g., 'Run'). Without an explicit `aria-label`, screen reader users miss context about what is being run and where it leads (new tab).
**Action:** When creating floating/overlay interactive elements with brief visual text, explicitly define a descriptive `aria-label` explaining the target and behavior (especially if they open new tabs), and ensure they have explicit `:focus-visible` styles matching their custom shapes.
