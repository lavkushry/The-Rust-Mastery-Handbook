...
## 2026-07-09 - Focus Rings and Aria-Labels on Overlay Buttons
**Learning:** When implementing custom link-based overlay buttons (e.g., 'Run' buttons floating on code blocks), explicitly define `:focus-visible` styles to match their custom border-radius/positioning, and ensure they have a descriptive `aria-label` (especially if they open new tabs) to supplement brief visual text.
**Action:** Always check custom absolute-positioned buttons to ensure their focus indicator isn't cut off or oddly shaped compared to their bounds, and supply rich context via `aria-label` for screen readers.
