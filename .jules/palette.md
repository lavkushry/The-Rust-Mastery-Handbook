...
## 2024-05-26 - CSS Pseudo-elements and Interactive Components
**Learning:** Avoid using CSS pseudo-elements (like `::after` or `::before`) to inject textual hints or icons into interactive UI components. Pseudo-elements cannot be individually hidden from screen readers using `aria-hidden`, leading to redundant and noisy announcements when focusing on the interactive element.
**Action:** When adding visual hints (like "tap to flip") or state icons (like checkmarks) to interactive components, inject actual DOM elements instead of pseudo-elements. This allows you to apply `aria-hidden="true"` to them and prevent screen reader clutter.
