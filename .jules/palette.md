...
## 2024-05-26 - Screen Reader Interpretation of Unicode Symbols
**Learning:** Unicode arrows (like `↗`, `←`, and `→`) and symbols (like `✓`) within text content are read literally by screen readers (e.g., "North East Arrow", "Check Mark"), which can be confusing inside button text or navigation elements.
**Action:** Always set an overriding `aria-label` with clear, human-readable text on interactive elements containing these symbols to prevent confusing screen reader announcements, especially for navigation or state actions.
