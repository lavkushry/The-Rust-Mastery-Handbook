...
## 2026-05-26 - Hidden Keyboard Hints for Custom Components
**Learning:** When adding visual hints for keyboard shortcuts to custom components (like flip cards mapped to `role="button"`), it's important to use `aria-hidden="true"` on the visual hint. Screen reader users inherently know how to interact with button semantics, and announcing custom keyboard hints clutters their experience.
**Action:** Always use `aria-hidden="true"` on visual keyboard hints when the underlying component has correct semantic ARIA roles.
