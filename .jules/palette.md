...
## 2024-05-26 - Literal Reading of Unicode Symbols
**Learning:** Unicode arrows (like `↗`, `←`, and `→`) and symbols like checkmarks (`✓`) within text content are read literally by screen readers (e.g., 'North East Arrow' or 'Check mark'), which can be confusing or redundant.
**Action:** Always set an overriding `aria-label` with clear, human-readable text on interactive elements containing these symbols to prevent confusing screen reader announcements, and use `aria-hidden="true"` on purely decorative symbols.
