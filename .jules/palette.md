...
## 2024-05-26 - Literal Unicode Symbol Screen Reader Announcements
**Learning:** Unicode arrows (like `↗`, `←`, and `→`) and symbols like checkmarks (`✓`) within text content are read literally by screen readers (e.g., 'North East Arrow' or 'Check mark'). This can be very confusing for interactive elements, e.g. reading 'Leftwards Arrow Prev' instead of just 'Previous'.
**Action:** Always set an overriding `aria-label` with clear, human-readable text on interactive elements containing these symbols to prevent confusing screen reader announcements. For elements that temporarily change state and symbols (like a success checkmark), dynamically update the `aria-label` to describe the new state and remove it when reverting.
