...
## 2024-05-26 - Unicode Arrows in Interactive Content
**Learning:** Unicode arrows (like `↗`, `←`, and `→`) within text content are read literally by screen readers (e.g., 'North East Arrow' or 'Leftwards Arrow'). When these characters are used in button labels (like '← Prev'), they create a confusing and verbose experience for screen reader users.
**Action:** Always set an overriding `aria-label` with clear, human-readable text on interactive elements containing these symbols to prevent confusing screen reader announcements.
