...
## 2024-06-07 - CSS Pseudo-Elements and Screen Reader Hints
**Learning:** Avoid using CSS pseudo-elements (like `::after` or `::before`) to inject textual hints or keyboard shortcuts into interactive UI components. Screen readers often announce the text from pseudo-elements, and because they are not real DOM nodes, they cannot be individually hidden from screen readers using `aria-hidden`. This causes redundant or confusing announcements.
**Action:** Inject actual DOM elements for textual hints and keyboard shortcuts, and explicitly add `aria-hidden="true"` to them so they don't clutter screen reader announcements, since screen reader users already know how to interact with native/semantic controls.
