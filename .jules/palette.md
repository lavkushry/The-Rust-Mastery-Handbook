...
## 2024-06-18 - Avoiding CSS Pseudo-elements for Textual Hints
**Learning:** Avoid using CSS pseudo-elements (like `::after` or `::before`) to inject textual hints or keyboard shortcuts into interactive UI components. These cannot be individually hidden from screen readers using `aria-hidden` and cause redundant announcements.
