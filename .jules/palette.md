...
## 2024-05-18 - Replacing Pseudo-elements for Hints
**Learning:** Screen readers might loudly announce text content inside CSS pseudo-elements (`::after`, `::before`) used as hints or icons. Hiding them specifically from screen readers isn't reliably possible because pseudo-elements don't accept `aria-hidden` attributes.
**Action:** Always inject actual DOM elements (like `<div>` or `<span>`) with `aria-hidden="true"` when providing supplementary visual hints, text, or icons that would be redundant or confusing for screen reader users.
