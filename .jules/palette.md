...
## 2024-06-12 - CSS Pseudo-elements and Screen Readers
**Learning:** CSS pseudo-elements (`::before` and `::after`) that inject text content (e.g. `content: "tap to flip ->"`) cannot be individually targeted with `aria-hidden="true"`. Screen readers will read the content injected by CSS pseudo-elements, leading to redundant or annoying announcements (especially for visual hints or icons like checkmarks) if they are on elements whose content is already announced or interactive.
**Action:** Always inject explicit DOM elements (`<span>` or `<div>`) for visual hints, keyboard shortcuts, or icons instead of using CSS `content:` rules. This allows adding `aria-hidden="true"` so screen readers can skip the redundant noise.
