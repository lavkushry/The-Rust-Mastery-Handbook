...
## 2024-05-26 - Literal Screen Reader Announcements of Unicode Symbols
**Learning:** Unicode arrows (like `←` and `→`) and symbols like checkmarks (`✓`) within text content are read literally by screen readers (e.g., 'Check mark' or 'Left arrow'), which can be confusing in interactive contexts like 'Mark chapter complete Check mark'.
**Action:** Always set an overriding `aria-label` with clear, human-readable text on interactive elements containing these symbols to prevent confusing screen reader announcements. For interactive elements that temporarily change text and visual symbols to indicate a transient state (like an export success checkmark), dynamically update the `aria-label` to describe the new state and revert or remove it when the element returns to its original state.
