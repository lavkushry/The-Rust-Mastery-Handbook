...
## 2024-05-18 - Avoid pseudo-elements for textual hints on interactive elements
**Learning:** Screen readers cannot independently ignore CSS pseudo-elements (like `::after` or `::before`) on an element. Therefore, injecting textual hints (like "tap to flip") or state symbols (like "✓") via pseudo-elements causes redundant announcements and clutter for screen reader users.
**Action:** Always inject actual DOM elements for hints/symbols and use `aria-hidden="true"` on them, keeping the visual cues while making the experience cleaner for assistive technologies.
