...
## 2024-05-26 - Screen Reader Noise from Pseudo-elements
**Learning:** Avoid using CSS pseudo-elements (like `::after` or `::before`) to inject textual hints or keyboard shortcuts into interactive UI components. Screen readers read these pseudo-elements along with the text content. They cannot be individually hidden from screen readers using `aria-hidden` and cause redundant announcements.
