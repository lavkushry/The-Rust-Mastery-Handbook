...
## 2024-05-26 - Custom Link Overlays and Focus Visibility
**Learning:** Custom interactive elements added dynamically to the content area (like absolute positioned overlay "Run" links on code blocks) do not inherit default focus styles and often have low contrast against their backgrounds if default styles are removed.
**Action:** When implementing custom link-based overlay buttons, always explicitly define `:focus-visible` styles to ensure they are keyboard accessible and visually distinct when focused.
