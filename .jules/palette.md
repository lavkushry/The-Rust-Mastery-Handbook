...
## 2024-05-14 - Hiding Literal Unicode Symbols from Screen Readers
**Learning:** Unicode arrows (like ↗, ←, and →) and symbols like checkmarks (✓) within text content are read literally by screen readers (e.g., "Leftwards Arrow Prev" or "Mark chapter complete Check mark"), confusing users. Also, "aria-hidden" is effective for pure decorative unicode symbols in sidebar links.
**Action:** Always set an overriding aria-label with clear, human-readable text on interactive elements containing these symbols, and use aria-hidden="true" on purely decorative elements like sidebar checkmarks.
